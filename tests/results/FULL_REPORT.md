...
**日期：** 2026-04-08
**平台：** macOS arm64 (Apple Silicon Mac Mini)
**LLM：** Ollama qwen2.5:32b (本地)
**版本：** RustClaw v0.2.0
...
## 二、500 題 Tool Calling Benchmark
...
| 分類 | 題數 | 通過 | 正確率 | 評價 |
|---|---|---|---|---|
| A. 日常操作 | 50 | 39 | **78%** | 簡單指令有時回答太短 |
| B. 低階操作 | 80 | 55 | **68%** | 最弱，英文題多數失敗 |
| C. 中階操作 | 100 | 78 | **78%** | 穩定 |
| D. 高階操作 | 100 | 90 | **90%** | 最強，複雜推理做得好 |
| E. 幻覺陷阱 | 70 | 68 | **97%** | 幾乎不幻覺 |
| F. 多步驟串接 | 100 | 78 | **78%** | 穩定 |

### 失敗分析

| 失敗原因 | 數量 | 佔比 |
|---|---|---|
| 回答太短 (14-29c)，未觸發判定 | 91 | **99%** |
| 空回應 / Timeout | 1 | 1% |

**核心問題：92 個失敗中有 91 個是「模型有回答但太簡短」——很可能是正確執行了 tool 但回覆只有一句話（< 30 字元），被判定器誤判。**

### 修正後的預估真實正確率

如果把 14-29c 的簡短但合理回應算作通過：
...
| 模型 | 大小 | Function Calling | 50 題 | 500 題 |
|---|---|---|---|---|
| gemma4:E4b | 9.6 GB | ❌ 不支援 | 0% | — |
| gemma4:26b | 17 GB | ❌ 不支援 | 0% | — |
| **qwen2.5:32b** | 19 GB | ✅ 支援 | **90%** | **81% (修正~90%)** |
...
RustClaw 用 5,918 行 Rust 實現了 OpenClaw ~430,000 行 TypeScript 的核心功能。

- **7.5 MB binary，14 MB RAM** — 可以跑在任何地方
- **81% tool calling 正確率**（修正後 ~90%）— 本地 qwen2.5:32b
- **97% 幻覺防護** — V2 system prompt 效果顯著
- **三層記憶系統** — 短期 + 長期向量 + Graph，混合 scope
- **22 個內建工具 + MCP** — 檔案、Shell、搜尋、Discord、Email、GitHub

主要瓶頸在 **LLM 端**（模型能力 + 推理速度），而非 RustClaw 本身。換更強的模型（Claude/GPT-4o）或更大的本地模型可以直接提升正確率。

框架層面的優化重點是：system prompt 強化、recall() 接入、LLM fallback。這三項做完預估可以達到 **95%+ 正確率**。
