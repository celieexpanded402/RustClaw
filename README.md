...
OpenClaw is a powerful personal AI assistant. It connects to every messaging platform you use, runs a local gateway, and gives you a real AI agent on your own hardware. But it comes with a cost: Node.js runtime, hundreds of megabytes of dependencies, and memory usage that rivals a small database.
...
So we built RustClaw — the 80/20 version of OpenClaw, written entirely in Rust, with zero human-written code. Every line was generated by [Claude Code](https://claude.ai/code) in a single session.

The architecture is intentionally minimal and modular. Two channels today — Telegram and Discord. A tool calling loop that can read files, write code, run commands, and open pull requests. A WebSocket gateway compatible with the OpenClaw protocol. Support for both Anthropic and any OpenAI-compatible endpoint, including local Ollama models.
